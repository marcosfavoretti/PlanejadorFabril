...
describe("GerenciadorPlanejamento.addPlanejamento", () => {
    ...
    beforeEach(() => {
        ...
        fakeCalendario = {
            inicioDoDia: jest.fn().mockImplementation((d) => new Date(d.setHours(0, 0, 0, 0))),
            finalDoDia: jest.fn().mockImplementation((d) => new Date(d.setHours(23, 59, 59, 999))),
        };
    ...
    it("should add planejamento to existing PlanejamentoDiario", async () => {
        ...
        planejamentoDiarioRepo.findOne.mockResolvedValue(existingPlanejamentoDiario);
        planejamentoDiarioRepo.save.mockResolvedValue({ ...existingPlanejamentoDiario, planejamentos: [{}] });

        const result = await gerenciador.addPlanejamento(fakePedido, fakeItem, fakeQtd, fakeSetor, new Date(fakeDate));
        ...
        expect(planejamentoDiarioRepo.save).toHaveBeenCalledWith(existingPlanejamentoDiario);
    ...
    it("should create and save a new PlanejamentoDiario if none exists", async () => {
        ...
        planejamentoDiarioRepo.save.mockResolvedValue({ dia: fakeDate, planejamentos: [{}] });
        ...
        expect(planejamentoDiarioRepo.create).toHaveBeenCalledWith({
            dia: expect.any(Date),
            planejamentos: [expect.objectContaining({
                item: fakeItem,
                pedido: fakePedido,
                qtd: fakeQtd,
                setor: fakeSetor,
    ...
    it("should throw an error if repository throws", async () => {
        ...
        await expect(
            gerenciador.addPlanejamento(fakePedido, fakeItem, fakeQtd, fakeSetor, new Date(fakeDate))
        ).rejects.toThrow(`problemas ao adicionar item ${fakeItem.codigo} no planejamento`);
    });
