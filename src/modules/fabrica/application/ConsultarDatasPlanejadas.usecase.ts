...
export class ConsultarDatasPlanejadasUseCase {
    ...
    async consultar(): Promise<Date[]> {
        try {
            ...
            if (!fabricaAtual) throw new Error('não foi possivel cosultar a fabrica atual');
            const planejamentoAtual = await this.consultaPlanejamentoService
                .consultaPlanejamentoAtual(fabricaAtual, new PlanejamentoOverWriteByPedidoService());
            // const planejamentos = await this.planejamentoRepository.find({ order: { dia: 'asc' } });

            const datas = planejamentoAtual
                .map(plan =>
            ...
            const uniqueDatas = Array.from(new Set(datas)).map(dateStr => new Date(dateStr));
