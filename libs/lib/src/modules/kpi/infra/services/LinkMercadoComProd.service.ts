...
export class LinkMercadoComProdService {
  ...
  async link(
    logEntries: [Date, Mercado[]][],
  ): Promise<GetMercadosEntreSetoresTabelaDto[]> {
    const response: GetMercadosEntreSetoresTabelaDto[] = [];
    const datesSet = new Set(
      logEntries.map((l) => l[0]).sort((a, b) => a.getTime() - b.getTime()),
    );
        ...
        for (const [partcode, qtd] of mercado.mercadoEntries()) {
          response.push({
            ...
            planejado:
              tabelaProducao.find(
                (t) =>
                  t.planejamento.dia.getTime() === dia.getTime() &&
