        ...
        try {
            ...
            const sortedItens = itens.sort(
                (a, b) => {
                    ...
                    return this.orderBySetoresChain(a.planejamento.setor.codigo, b.planejamento.setor.codigo);
                }
            );
    ...
    private legendaDeOperacao(): GanttLegendaDto[] {
        ...
        for (const setor of setoresOrdenados) {
            ...
            if (cor) {
                ...
                const enumKey = Object.keys(CODIGOSETOR).find(key => CODIGOSETOR[key as keyof typeof CODIGOSETOR] === setor);
                legenda.push({
