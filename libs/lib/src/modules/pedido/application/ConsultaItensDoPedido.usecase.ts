...
export class ConsultaItensDoPedidoUseCase {
    ...
    async consulta(dto: ConsultartPorPedidoDto): Promise<ItemResDto[]> {
        try {
            ...
            const estrutura = await this.montaEstrutura.monteEstrutura(pedido.getItem());
            return estrutura.itensAsList().map(ItemResDto.createByEntity);
        } catch (error) {
            throw new InternalServerErrorException(`Nao foi possível consultar os itens do pedido \n${error}`);
        }
    }
}