...
export class ValidaCapacidade implements IValidaPlanejamento {
  constructor(
    ...
    @Inject(forwardRef(() => IGerenciadorPlanejamentConsulta))
    private gerenciamentoConsulta: IGerenciadorPlanejamentConsulta,
  ) {}
    ...
    for (const temp of planejamentosTemp) {
      ...
      if (!podeAlocar) {
        throw new ErroDeValidacao(
          `Tentativa de alocar carga excedente no dia ${temp.dia.toISOString()} setor ${temp.setor}`,
        );
      }
    }
  }
