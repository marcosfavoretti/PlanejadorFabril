...
export class SetorChainFactoryService {
  ...
  setMetodoDeAlocacaoCustomTodos(
    pipeProducaoCustom: SetorService,
    setores: CODIGOSETOR[],
  ...
  setMetodoDeAlocacaoCustomUnico(
    pipeProducaoCustom: SetorService,
    setor: CODIGOSETOR,
    metodoDeAlocacao: MetodoDeAlocacao,
  ): SetorService {
  ...
  setMetodoDeRealocacaoCustomTodos(
    pipeProducaoCustom: SetorService,
    setores: CODIGOSETOR[],
    metodoDeReAlocacao: MetodoDeReAlocacao,
  ): SetorService {
    const pipeAsList = pipeProducaoCustom.getSetoresInChain(setores);
    pipeAsList.forEach((pipe) =>
      pipe.setMetodoDeReAlocacao(metodoDeReAlocacao),
    );
    return pipeProducaoCustom;
  }

  setMetodoDeRealocacaoCustomUnico(
    pipeProducaoCustom: SetorService,
    setor: CODIGOSETOR,
    metodoDeReAlocacao: MetodoDeReAlocacao,
  ): SetorService {
   ...
   */
  modificarCorrente(
    codigosAlvos: CODIGOSETOR[],
    corrente?: SetorService,
  ): SetorService {
    const setoresService = (
      corrente || this.pipeProducao
    ).getSetoresInChainClone(codigosAlvos);
    for (let i = 0; i < setoresService.length - 1; i++) {
      setoresService[i].setNextSetor(setoresService[i + 1]);
    }
    this.logger.log(
      `___SETOR MODIFICADO___\n${setoresService.map((s) => s.getSetorCode())}`,
      'SETOR IN CHAIN FACTORY',
    );
