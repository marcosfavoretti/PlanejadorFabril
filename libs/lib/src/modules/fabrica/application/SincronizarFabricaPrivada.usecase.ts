...
export class SincronizarFabricaPrivadaUseCase {
    ...
    async sincroniza(dto: SincronizarFabricaPrivadaDTO): Promise<FabricaResponseDto> {
        try {
            const [fabricaPrincipal, fabrica] = await Promise.all([this.fabricaService.consultaFabricaPrincipal(), this.fabricaService.consultaFabrica(dto.fabricaId)]);
            ...
            Logger.error(`não foi possível sincronizar a fabrica principal com a fabrica ${dto.fabricaId}`);
