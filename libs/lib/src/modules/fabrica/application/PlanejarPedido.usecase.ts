...
export class PlanejarPedidoUseCase {
  ...
  @Inject(OnNovoPlanejamento)
  private onNovoPlanejamento: IOnNovoPlanejamentos[];
  ...
  async planeje(dto: InputPedidosDTO): Promise<void> {
    try {
      ...
      if (!fabricaPrincipal)
        throw new Error('Não foi encontrado a fabrica principal');
      ...
      for (const pedido of pedidos) {
        ...
        if (!fabricaVersionada) {
          fabricaVersionada = await this.fabricaService.transitionSave(
            fabricaNovaParaMudanca,
          );
        }
        //
        ...
        const planejamentos =
          await this.gerenciadorPlanejamentoMutation.appendPlanejamento(
            fabricaVersionada,
            pedido,
      ...
      throw new InternalServerErrorException(
        `Não foi possível consultar os pedidos ou aloca-los.\n ${error.message}`,
      );
    }
  }
}
