...
export class DesplanejarPedidoUseCase {
    ...
    async desplanejar(dto: DesplanejarPedidoDto, user: User): Promise<void> {
        try {
            ...
            const primeiroPlanejamento = planejamentosSnapShot.sort((a, b) => a.planejamento.dia.getTime() - b.planejamento.dia.getTime())[0]
            if (isBefore(primeiroPlanejamento.planejamento.dia, startOfTomorrow())) {
                throw new Error('o planejamento ja foi realizado, impossivel apagar o pedido')
            }
            ...
            const pedidosRemovidos = pedidosConvertidos.map(pedido => ({ ...pedido, processado: false }));
