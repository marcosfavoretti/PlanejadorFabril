...
export class ReplanejarPedidoUseCase {
  constructor(
    ...
    @Inject(ApagaPedidoPlanejadoService)
    private apagaPedidoPlanejadoService: ApagaPedidoPlanejadoService,
    ...
    private fabricaSimulacao: FabricaSimulacaoService,
  ) {}
  ...
  async replanejar(dto: ReplanejarPedidoDTO): Promise<void> {
    ...
    await Promise.all([
      this.apagaPedidoPlanejadoService.apagar(fabrica, pedido),
      this.gerenciaDividaService.apagarDividas(fabrica, pedido),
    ]);
