  ...
  async remover(dto: RemoverPlanejamentoDTO): Promise<void> {
    try {
      ...
      if (fabrica.principal)
        throw new Error('Essa fabrica nao deve ser alterada');

      const planejamento = await this.planejamentoService.consultaPlanejamento(
        dto.planejamentoId,
      );
      ...
      await this.gerenciadorPlanejamento.removePlanejamento(fabrica, [
      ...
      const divida = await this.gerenciaDividaService.resolverDividas({
        fabrica,
        pedido: planejamento.pedido,
        planejamentos: planejamentosDaFabrica.map(
