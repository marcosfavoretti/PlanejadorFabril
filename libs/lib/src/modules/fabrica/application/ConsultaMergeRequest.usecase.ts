...
export class ConsultaMergeRequestUseCase {
    ...
    async consultar(): Promise<MergeRequestPendingDto[]> {
        try {
            ...
            const upToDateMergeRequest = await this.mergeRequestService.findMergeByFather(fabricaMain!);
            return upToDateMergeRequest.map(up => MergeRequestPendingDto.createByEntity(up));
        } catch (error) {
