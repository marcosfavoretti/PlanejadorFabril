  ...
  implements IGerenciadorPlanejamentoMutation, IGerenciadorPlanejamentConsulta
{
  ...
  constructor(
    @Inject(PlanejamentoValidatorExecutorService)
    private planejamentoValidatorExecutor: PlanejamentoValidatorExecutorService,
  ...
  async appendReplanejamento(
    ...
    const resultados = await this.efetivaPlanejamentoService.efetiva(
      fabrica,
      planejamentosNovos,
    );
  ...
  async removePlanejamento(
    ...
    const planPromises = planejamento.map((m) =>
      this.efetivaPlanejamentoService.remove(fabrica, m),
    );
  ...
  async appendPlanejamento(
    ...
    try {
      ...
      for (const planejamentoTemp of planejamentosTemp) {
        const planejamentosSemelhantes = noBancoAtual
          .filter(
            (bd) =>
          ...
          .reduce(
            (total, planSnapShot) => (total += planSnapShot.planejamento.qtd),
            0,
          );
  ...
  async diaParaAdiarProducaoEncaixe(
    ...
    for (
      let tentativa = 0;
      tentativa < 2_000 && quantoPreciso > 0;
      tentativa++
    ) {
      // já usei esse dia?
