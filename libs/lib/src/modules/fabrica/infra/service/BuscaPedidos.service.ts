...
export class BuscaPedidosService implements IBuscaAtraso {
  constructor(
  ...
  async buscaAtraso(
    ...
    const planejamentos = await this.consultaPlanejamento.consultaPorPedido(
      fabrica,
      pedidos,
  ...
  async buscaPedidosNaoIniciados(props: {
    ...
    const planejamentoQueJaForamIniciados = planejamentos.filter(
      (plan) => plan.planejamento.dia.getTime() <= startOfToday().getTime(),
    );
    const pedidosQueJaForamIniciados = new Set(
      planejamentoQueJaForamIniciados.map(
        (plan) => plan.planejamento.pedido.id,
      ),
    );
    const pedidos = new Set(
      planejamentos.map((plan) => plan.planejamento.pedido),
    );
    const pedidosNaoIniciados = Array.from(pedidos).filter(
      (ped) => !pedidosQueJaForamIniciados.has(ped.id),
    );
  ...
  async buscaPedidosComColisao(props: {
    ...
    const { pedido: pedidosNaoIniciados } = await this.buscaPedidosNaoIniciados(
      { fabrica },
    );
  ...
  async pedidosNaFabrica(fabrica: Fabrica): Promise<Pedido[]> {
    const planejamentos =
      await this.consultaPlanejamento.consultaPlanejamentoAtual(
        fabrica,
        new PlanejamentoOverWriteByPedidoService(),
      );
