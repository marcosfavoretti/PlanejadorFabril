...
export class EfetivaPlanejamentoService {
  ...
  async efetiva(
    ...
    try {
      ...
      const planejamentosParaSalvar = novosSnapshots.map(
        (snapshot) => snapshot.planejamento,
      );
      const insertResultPlanejamentos =
        await this.planejamentoRepository.insert(planejamentosParaSalvar);

      novosSnapshots.forEach((snapshot, index) => {
        const newId =
          insertResultPlanejamentos.identifiers[index].planejamentoId; // ou o nome correto da sua PK
        snapshot.planejamento.planejamentoId = newId;
      });

      const insertResultSnapshots =
        await this.planejamentoSnapShotRepository.insert(novosSnapshots);

      const ids = insertResultSnapshots.identifiers.map(
        (i) => i.planejamentoSnapShotId,
      );

      const planejamentosSalvos =
        await this.planejamentoSnapShotRepository.find({
          where: {
      ...
      console.error(error);
      throw new Error(
        `Problemas para salvar o planejamento: ${(error as Error).message}`,
      );
    }
  }
  ...
  private criarSnapshot(
    ...
    const dataToCreate = {
      ...
      planejamento: {
        ...
        setor: planejamentoTemp.setor
          ? { codigo: planejamentoTemp.setor }
          : undefined,
      },
      tipoAcao: acao,
    };
    ...
    try {
      ...
      Logger.fatal(
        '!!! ERRO DENTRO DO repository.create() !!!',
        (creationError as Error).stack,
      );
      throw creationError;
    }
  }
