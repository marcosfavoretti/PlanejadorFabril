...
export class DividaCheckPoint implements IGeraCheckPoint {
  ...
  async gerar(fabricaAtual: Fabrica, fabricaPassada: Fabrica): Promise<void> {
    const dividas =
      await this.dividaService.consultarDividaDaFabrica(fabricaPassada);
    fabricaAtual.appendDividas(dividas.map((divida) => divida.copy()));
  }
}
