...
export class FabricaSimulacaoService {
  constructor(
    ...
    @Inject(IConsultarRoteiroPrincipal)
    private consultaRoteiroPrincipal: IConsultarRoteiroPrincipal,
  ...
  async planejamento(
    fabrica: Fabrica,
    pedido: Pedido,
  ): Promise<FabricaPlanejamentoResultado> {
    const planejamentosTemporarios: PlanejamentoTemporario[] = [];
    try {
      const estrutura = await this.montaEstruturaEstrategia.monteEstrutura(
        pedido.item,
      );

      const planejamentoCorrente =
        await this.consultaPlanejamentoService.consultaPlanejamentoAtual(
          fabrica,
          new PlanejamentoOverWriteByPedidoService(),
        );

      const planejamentoCorrenteTemp = planejamentoCorrente.map(
        PlanejamentoTemporario.createByEntity,
      );

      // 1. Planejamento principal (já retorna o pipe também)
      const { planInicial, pipePrincipal } = await this.alocaItemPrincipal(
        fabrica,
        pedido,
        estrutura,
      ...
      if (pivot) {
        ...
        planejamentosTemporarios.splice(
          0,
          planejamentosTemporarios.length,
          ...planoAjustado,
        );
      }
      ...
      if (estrutura.itensDependencia.length > 0) {
        const plansDeps = await this.alocaDependencias(
          fabrica,
          pedido,
          estrutura,
          planejamentoCorrenteTemp,
      ...
      console.error('[planejamento] erro geral ao processar pedidos', {
        error,
      });
  ...
  private async gerarPipeDeProducao(
    estrutura: ItemEstruturado,
  ): Promise<SetorService> {
  ...
  private getPlanejamentoPivot(
    ...
    const foraDoRangeAsc = this.organizaPlansOrdemCronologica(
      planInicial,
      pipe,
    );
  ...
  private async alocaDependencias(
    ...
    for (const item of [...estrutura.itensDependencia]) {
      ...
      const pipe =
        this.setorChainFactoryService.modificarCorrente(setoresDoItem);
  ...
  async replanejarDependencia(props: {
    fabrica: Fabrica;
    estrutura: ItemEstruturado;
    ...
    const planejamentosDependentes = props.planejamentoPedido.filter(
      (plan) => plan.item === props.planejamentoFalho.item,
    );
    ...
    const pipeProducao =
      this.setorChainFactoryService.modificarCorrente(roteiroCompleto);
    ...
    [resultado.planejamentos, resultado.retirado] = [
      resultadoAlocacao.adicionado,
      resultadoAlocacao.retirado,
    ];
    return resultado;
  }
  ...
  async replanejar(
    fabrica: Fabrica,
    planejamentosFalhos: PlanejamentoTemporario[],
    novaData?: Date,
  ): Promise<FabricaReplanejamentoResultado> {
    const planejamentosTemporarios: PlanejamentoTemporario[] = [];
    ...
    try {
      for (const realocacao of planejamentosFalhos) {
        ...
        const planejamentosDoPedidoEmBanco =
        ...
        if (
          estrutura.itensDependencia
            .map((i) => i.Item)
            .includes(realocacao.item.Item)
        ) {
          ...
          const response = await this.replanejarDependencia({
            ...
            planejamentoPedido: planejamentosDoPedidoEmBanco.map(
              PlanejamentoTemporario.createByEntity,
            ),
            novaData,
          });
        ...
        const pipeSetoresCustom =
          this.setorChainFactoryService.modificarCorrente(roteiro);

        const setorAlvo = pipeSetoresCustom.getSetorInChain(realocacao.setor);

        const planDoPedidoPrincipal = planejamentosAlvos.filter((plan) =>
          [
            estrutura.itemFinal.getCodigo(),
            estrutura.itemRops.getCodigo(),
          ].includes(plan.planejamento.item.getCodigo()),
        );
        ...
        const { adicionado, retirado } = await setorAlvo.realocar({
          ...
          novoDia: novaData
            ? novaData
            : this.calendario.proximoDiaUtilReplanejamento(new Date()),
          pedido: realocacao.pedido,
          planFalho: realocacao,
          planejamentoFabril: planejamentoCorrenteTemp,
          planDoPedido: planDoPedidoPrincipal.map(
            PlanejamentoTemporario.createByEntity,
          ),
        });
      ...
      throw new Error(
        `problema ao salvar planejamentos temporarios: ${error.message}`,
      );
    }
  }

  private organizaPlansOrdemCronologica(
    planejamentos: PlanejamentoTemporario[],
    pipeSetores: SetorService,
  ...
  private selecionaPlansForaDoRange(
    planejamentos: PlanejamentoTemporario[],
  ): PlanejamentoTemporario[] {
    const planejamentosForaDoRange = planejamentos.filter((plan) =>
      isBefore(plan.dia, startOfTomorrow()),
    );
  ...
  private async arrumaRangeDoPlanejamento(props: {
    ...
    const primeiroSetorDoPipe = props.pipeProducao.getSetorInChain(
    ...
    const { adicionado: realocacao } = await primeiroSetorDoPipe.realocar({
      ...
      novoDia: this.calendario.proximoDiaUtilReplanejamento(
        props.planPivot.dia,
      ),
      pedido: props.pedido,
