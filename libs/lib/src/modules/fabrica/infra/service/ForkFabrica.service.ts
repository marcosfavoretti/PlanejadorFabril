...
export class ForkFabricaService {
  ...
  constructor(
    ...
    @Inject('CHECKPOINT_SERVICES')
    private checkpoints_services: Array<IGeraCheckPoint>,
  ) {
    Logger.debug(`CHECKPOINT EACH ${this.CHECKPOINT_RANGE}`);
  }

  async fork(props: ForkFabricaProps): Promise<Fabrica> {
    const fabricaCopy = props.fabrica.copy({
      user: props.user,
   ...
   */
  private async aplicarCheckpointSeNecessario(
    fabricaAtual: Fabrica,
    props: ForkFabricaProps,
  ): Promise<void> {
    const ancestrais = await this.fabricaService.consultarFabricasAteCheckPoint(
      props.fabrica,
    );
    if (
      props.forceCheckPoint ||
      (props.isPrincipal && ancestrais.length >= this.CHECKPOINT_RANGE)
    ) {
