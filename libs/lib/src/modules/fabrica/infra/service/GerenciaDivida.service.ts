...
import {
  ...
  ICalculoDivida,
} from '../../@core/interfaces/ICalculoDivida';
...
export class GerenciaDividaService implements ISetStrategy<ICalculoDivida> {
  constructor(
  ...
  async adicionaDividas(fabrica: Fabrica, dividas: Divida[]): Promise<void> {
    ...
    return await this.dividaService.addDivida(
      fabrica,
      dividas,
   ...
   * @returns
   */
  async resolverDividas(
    props: CalculaDividaDoPlanejamentoProps,
  ): Promise<Divida[]> {
    //esse metodo logo nao podera mais executar a interface dentro dele
    try {
      // TODO IMPROVE: para tentar deixar menos custoso isso
      const dividas = (await this.calculoDivida.calc(props)).filter(
        (divida) => divida.qtd > 0,
      );

      await this.dividaService.removerDividaNaFabricaDoPedido(
        props.fabrica,
        props.pedido,
      );
