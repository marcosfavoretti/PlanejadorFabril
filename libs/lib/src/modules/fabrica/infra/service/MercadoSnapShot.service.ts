...
export class MercadoSnapShotService {
  ...
  constructor(
    @Inject(MercadoSnapShotRepository)
    private mercadoSnapShotRepository: MercadoSnapShotRepository,
  ) {}
  ...
  async consultarMercados(fabrica: Fabrica): Promise<MercadoSnapShot[]> {
    ...
    const mercadoOverwrite =
      this.mercadoOverWriteService.manager(mercadoSnapShot);
