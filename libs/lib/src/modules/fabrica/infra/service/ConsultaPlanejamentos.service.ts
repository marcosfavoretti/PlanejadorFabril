...
export class ConsultaPlanejamentoService {
  ...
  async consultaPlanejamentoDoPedidoAteFabrica(
    fabrica: Fabrica,
    pedido: Pedido,
    ...
    return await new PlanejamentoOverWriteByPedidoService().resolverOverwrite(
      planejamentos,
    );
  }

  async consultaPlanejamentoDaFabricaPrincipal(): Promise<
  ...
  > {
    ...
    const fabricasAlvos =
      await this.fabricaService.consultarFabricasAteCheckPoint(
        fabricaPrincipal,
      );
  ...
  async consultaPlanejamentoEspecifico(
    fabrica: Fabrica,
    planejamento: Planejamento,
    overwriteStrategy: IGerenciaOverwrite<PlanejamentoSnapShot>,
  ): Promise<PlanejamentoSnapShot> {
    const fabricasAlvos =
    ...
    const [ultimoPlanejamento] =
      overwriteStrategy.resolverOverwrite(planejamentos);
    if (!ultimoPlanejamento)
      throw new Error(
        `Não existe o planejamento ${planejamento.planejamentoId}`,
      );
    return ultimoPlanejamento;
  }

  async consultarPlanejamentosFuturos(
    fabrica: Fabrica,
    overwriteStrategy: IGerenciaOverwrite<PlanejamentoSnapShot>,
  ): Promise<PlanejamentoSnapShot[]> {
    const fabricasAlvos =
  ...
  async planejamentoDoSetorNoDia(
    fabrica: Fabrica,
    setor: CODIGOSETOR,
    dia: Date,
    overwriteStrategy: IGerenciaOverwrite<PlanejamentoSnapShot>,
  ...
  async consultaPorPedido(
    ...
    try {
      ...
      throw new Error(
        `Problemas ao consultar fabrica .consultaPorPedido ${error}`,
      );
    }
  }

  async consultaPlanejamentoDia(
    fabrica: Fabrica,
    diaInicio: Date,
    overwriteStrategy: IGerenciaOverwrite<PlanejamentoSnapShot>,
    diaFinal?: Date,
    ...
    try {
      ...
      const snapShotCompleto = await this.planejamentoSnapShotRepository.find({
        where: {
          ...
          planejamento: {
            dia: Between(
              diaInicio,
              diaFinal || this.calendario.finalDoDia(diaInicio),
            ),
          },
        },
      });
      ...
      throw new Error(
        `Problemas ao consultar fabrica .consultaPlanejamentoDia ${error}`,
      );
    }
  }

  async consultaPlanejamentoAtual(
    fabrica: Fabrica,
    overwriteStrategy: IGerenciaOverwrite<PlanejamentoSnapShot>,
  ): Promise<PlanejamentoSnapShot[]> {
    try {
      ...
      throw new Error(
        `Problemas ao consultar fabrica .consultaPlaenjaemntoAtual ${error}`,
      );
    }
  }

  async consultaPlanejamentoSnapShot(
  ...
  async consultaPlanejamentoSnapShots(
    planejamentoSnapShotId: number[],
  ): Promise<PlanejamentoSnapShot[]> {
   ...
   */
  async consultaItemNoSetorNoDia(
    fabrica: Fabrica,
    item: Item,
    setor: CODIGOSETOR,
    dia: Date,
    ...
    try {
      ...
      const snapshots = overwriteStrategy.resolverOverwrite(snapShotCompleto);
      return snapshots;
    } catch (error) {
