...
export class PlanejamentoCheckPoint implements IGeraCheckPoint {
  ...
  async gerar(fabricaAtual: Fabrica, fabricaPassada: Fabrica): Promise<void> {
    const planejamentos =
      await this.consultaPlanejamento.consultarPlanejamentosFuturos(
        fabricaPassada,
        new PlanejamentoOverWriteByPedidoService(),
      );
    fabricaAtual.appendPlanejamento(planejamentos.map((plan) => plan.copy()));
  }
}
