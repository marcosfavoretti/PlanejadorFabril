...
export class VirtualDateService implements OnModuleInit {
  constructor(
    @Inject(VirtualDateRepository)
    private virtualDateRepo: Repository<VirtualDate>,
  ) {}
  ...
  async praFrente(): Promise<Date> {
    try {
      ...
      const novoDia = this.calendario.proximoDiaUtil(
  ...
  async paraTras(): Promise<Date> {
    try {
      ...
      const novoDia = this.calendario.ultimoDiaUtil(
        virtualDate.virtualDate,
        false,
      );
      virtualDate.virtualDate = novoDia;
