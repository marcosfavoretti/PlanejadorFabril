  ...
  async valide(fabrica: Fabrica): Promise<void> {
    ...
    for (const pedido of pedidosDaFabrica) {
      ...
      if (!planejamentoDoPedido) {
        ...
        Logger.warn(
          `Nenhum planejamento encontrado para o pedido ${pedido.id}`,
        );
      }
      ...
      const dividas = await this.calculoDivida.calc({
        ...
        pedido,
        planejamentos:
          planejamentoDoPedido?.map(PlanejamentoTemporario.createByEntity) ??
          [], // Garante que seja um array vazio se não houver planejamento
      });
      ...
      if (dividas.length > 0) {
        ...
        throw new FabricaPlanejadaErradaException(
          fabrica,
          pedido,
          discrepancias,
        );
