...
export class CargoService implements ISetUserCargo, IGetCargo, IGetUserCargos {
  constructor(
    @Inject(CargoRepository) private cargoRepository: CargoRepository,
    @Inject(GerenciaCargoRepository)
    private gerenciaCargoRepository: GerenciaCargoRepository,
  ) {}
  ...
  async setUserCargo(user: User, cargo: CargoEnum): Promise<GerenciaCargo> {
    try {
      ...
      throw new Error(
        `Falha a atribuir cargo para o usuario ${user.name} ${cargo}`,
      );
    }
  }
}
