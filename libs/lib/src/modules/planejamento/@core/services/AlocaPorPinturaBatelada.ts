...
export class AlocaPorPinturaBatelada extends MetodoDeAlocacao {
    private readonly PINTURAMAX!: number;
    ...
    verificacaoCapacidade(pedido: Pedido, codigoSetor: CODIGOSETOR): IVerificaCapacidade {
        return new VerificaBatelada(this.PINTURAMAX);
    }
    ...
    protected async alocacaoComDependencia(props: AlocacaoComDependenciaProps): Promise<PlanejamentoTemporario[]> {
        ...
        for (const necessidade of planejamentosProxOrdenados) {
            ...
            const datasParaProgramar = await this.gerenciadorPlan.diaParaAdiantarProducaoEncaixe(
                props.fabrica, dataLimite, props.setor, props.pedido.item, precisoAlocar, new VerificaBatelada(this.PINTURAMAX), planejamentosTemporarios
            );
            ...
            for (const data of datasParaProgramar) {
                const response = await this.gerenciadorPlan.possoAlocarQuantoNoDia(props.fabrica, data, props.setor, props.pedido.item, new VerificaBatelada(this.PINTURAMAX), planejamentosTemporarios);
