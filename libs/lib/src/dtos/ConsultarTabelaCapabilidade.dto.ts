...
export class ConsultarTabelaCapabilidadeDTO {
    ...
    static fromEntities(item: Item): ConsultarTabelaCapabilidadeDTO {
        const capabilidade_00011 = item.itemCapabilidade.find(cap => cap.setor.codigo === CODIGOSETOR.DOBRA);
        const capabilidade_00015 = item.itemCapabilidade.find(cap => cap.setor.codigo === CODIGOSETOR.SOLDA);
        const capabilidade_00017 = item.itemCapabilidade.find(cap => cap.setor.codigo === CODIGOSETOR.LIXA);
        const capabilidade_00025 = item.itemCapabilidade.find(cap => cap.setor.codigo === CODIGOSETOR.MONTAGEM);
        const capabilidade_00035 = item.itemCapabilidade.find(cap => cap.setor.codigo === CODIGOSETOR.BANHO);
        const capabilidade_00050 = item.itemCapabilidade.find(cap => cap.setor.codigo === CODIGOSETOR.PINTURALIQ);
